...
layout(location = 0) uniform sampler2D norm1tex;
layout(location = 1) uniform sampler2D norm2tex;
...
void main()
{
	...
	vec2 distortedTexCoords = (texture(norm1tex, vec2(fragTex.x+moveFactor, fragTex.y)).rg * 2.0 - 1.0) * 0.02 ;
	vec2 distortedTexCoords2 = (texture(norm1tex, vec2(-fragTex.x+moveFactor, fragTex.y+moveFactor)).rg * 2.0 - 1.0) * 0.02;
	vec2 totalDistortion = distortedTexCoords + distortedTexCoords2;
	vec4 normalMapColor = texture(norm1tex, totalDistortion);
